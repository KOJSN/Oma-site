...
/* ══ the name that was only ever on the phone ═════════════════════════
   ...
   app_user.full_name was empty for every person who has ever signed up,
   and the reason was not one bug but two sitting on top of each other.

   The signup screen — `saveMe`, below — wrote DB.me to localStorage and
   navigated away. No server call at all. And the profile screen, which
   DOES call API.saveProfile, was calling api_save_profile, which did not
   exist in the database: the request 404'd, the catch fired, and the
   person saw "Saved here, but not on the server", which nobody reads as
   "your name is gone". people.sql creates that function; this sends it
   something to store.

   Called in three places, because a name is typed before there is
   anywhere to put it: at signup, at sign-in (the first moment a session
   exists to save it against), and at boot, which is what carries the
   names already sitting on ten phones up to the server the next time
   those people open Oma.

   Best effort, always. The local copy is saved before this runs, so a
   failed round trip costs nothing and must never raise a dialog. */
function pushProfile() {
  ...
  if (a === "saveMe") {
    ...
    delete DB.me.ll;      // a customer has no pin; see myPos()
    dbSave();
    pushProfile();        // …and on the server, which this never did
...
(function boot() {
  ...
  noteWhereTheyJoined();
  // The ten names already sitting in localStorage on ten phones, carried up
  // the first time each of those people opens Oma again. See people.sql.
  pushProfile();
})();
