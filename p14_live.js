  ...
  load(async () => {
    ...
    fillHost(`
      <div class="pad stack gap12">
        ...
        <!-- Kamsy, 20 Sep 2026: "for no reason can the customer message when
             she hasn't paid, or report either." Before payment there is
        ...
        <!-- No cancel button, deliberately. An unpaid hold expires on its own
             ...
             after the appointment (refund-window.sql). A free cancel button
             is a way to hold a tech's slot and drop it at no cost, over and
             over; making a pest pay first and wait for it back is the
             deterrent. -->

        <!-- Both sides, through the same door.
             Kamsy, 20 Sep 2026: "as long as the appointment has occurred the
             customer can never report anything." Once a booking is
             'released' the appointment has happened AND been paid for —
             that status only exists because the tech scanned the customer's
             own code, which is proof the two of them really met. There is
             nothing left to report at that point that this button could act
             on, so it disappears exactly there. Also gone before payment,
             for the same "nothing has happened yet" reason as the chat
             button above.

             Kamsy, 20 Sep 2026 (later the same day): "customers can't be
             able to report till after a service is finished." So it is not
             enough that the booking is merely 'paid' — the door only opens
             once the appointment's own time has actually run out, service
             end = starts_at + minutes. Before that, whatever "went wrong"
             hasn't happened yet, or is still happening, and this is quiet
             specifically so a loud one cannot be used as a threat mid
             appointment. A no-show is still reportable here as "Did not
             turn up" — a no-show is only knowable once the booked time is
             over anyway, so this same rule already covers it. -->
        ${(() => {
          if (["awaiting_payment", "released"].includes(b.status)) return "";
          const finishesAt = at + Math.max(0, b.minutes || 0) * 60000;
          if (Date.now() < finishesAt) return "";
          return `
...
/* THE SCOPE PILL IS GONE. It floated over the top of the map, which on the
   home screen means over the top of the scan card — a control almost nobody
   needed, covering the sentence that explains what a hand scan is. Two
   permanent buttons for a question that only arises when the map is empty.

   Where it went: "See other states" now lives in the empty state, which is
   the only moment the question is real, and the heading below carries the way
   back. Kamsy, 16 Sep 2026: "remove the near me and other states features but
...
function drawMap(pos, list) {
  ...
  //
  // Kamsy, 23 Sep 2026, from a screenshot: Ayo studio (10 m away, no shop)
  // was right there in the list under the sheet but never showed on the map
  // at all — only Emma studio's (37 m away) pin did. Nothing was wrong with
  // Ayo's data. hex.sql's "shown" grid rounds a published position to
  // roughly a 600 m cell on purpose, for privacy (a room in someone's house
  // must never be pinpointed) — so any two techs that close land on the
  // EXACT same published point, not just a nearby one. Two markers on the
  // identical pixel is one marker, and whichever got added to the map last
  // simply sits on top of the other, hiding it completely. This has nothing
  // to do with having a shop or not; it will happen to any two techs inside
  // the same rounding cell. Group the list by that shared point and nudge
  // duplicates into a small ring around it — a purely visual spread, not a
  // change to the coordinate the privacy rounding produced — so every tech
  // in a crowded cell gets her own visible, tappable pin.
  ...
  groups.forEach((group) => {
    group.forEach((t, i) => {
      ...
      if (group.length > 1) {
        // An 18 m ring — enough to separate a 36px pin from its neighbours
        // at any zoom the map opens at, still well inside the 600 m cell
        // they were already rounded to.
        const ang = (2 * Math.PI * i) / group.length;
        lat += (18 * Math.cos(ang)) / 111320;
        lng += (18 * Math.sin(ang)) / (111320 * Math.cos(lat * Math.PI / 180));
      }
      const m = L.marker([lat, lng], { icon: pinIcon(t, false), title: t.business_name })
        .addTo(MAP);
...
function sheetMany() {
  ...
  return `
    ...
    <div class="rowbetween" style="margin-bottom:8px">
      <div class="lbl" style="margin:0">${MSCOPE.all
        ? `Nail techs in ${esc(MSCOPE.state || "that state")}`
        : n ? `${n} nail tech${n > 1 ? "s" : ""} near you`
            : "Nail techs near you"}</div>
      <!-- The return journey. Without the pill, somebody who tapped through to
           a state and FOUND people had no way back — the empty state's "Back
           to near me" only appears when there is nothing there. -->
      ${MSCOPE.all
        ? `<span class="seeall" data-a="map-scope" data-v="near">Near me</span>`
...
/* ── the way out of an empty map ──────────────────────
   "No nail techs near you yet" and then nothing was a dead end, and it is the
   commonest screen Oma has: a marketplace is empty everywhere until it is full
   somewhere. A woman in Kano with nobody listed near her got a dashed box and
   no next move, when Oma had techs in Lagos the whole time.

   So the empty state carries the answer to "well, where ARE they?" — one tap
   to the busiest state, using the scope the map already has rather than a new
   idea. MSTATES is already loaded for the state picker; this reads the same
   list and needs no extra request.

   It is a LINK, not a button. A filled pink slab inside a dashed empty box is
   two competing shapes arguing in a space the size of a postcard, and it made
   the absence of nail techs look like the main event on the screen. The map is
   the screen; this is a footnote on it. And it says "other states" rather than
   naming one, because the point being made is that Oma has people elsewhere —
   which state is a detail she can change in the pill once she is there.

   It also settles the App Store problem. App Review runs in California: a
   reviewer who taps Allow on the location prompt gets Cupertino coordinates
   and an empty map, which reads as a broken app and is a Guideline 2.1
   rejection. Oma's fallback to Lagos Island only fires when location FAILS,
   so it never helps the reviewer who says yes. This does, and it does it by
   being the finished version of the screen rather than a thing bolted on for
   Apple. */
function busiestState() {
  if (!MSTATES || !MSTATES.length) return null;
  return MSTATES.reduce((a, b) => (Number(b.techs) || 0) > (Number(a.techs) || 0) ? b : a);
}

function emptyNear(pad) {
  const b = busiestState();
  // Already looking at a whole state, or nothing is listed anywhere yet. In
  // both cases there is nowhere better to send her, and a button that goes
  // somewhere equally empty is worse than no button.
  const out = !MSCOPE.all && b && (Number(b.techs) || 0) > 0;
  return `<div class="empty"${pad ? ` style="padding:18px 12px"` : ""}>
    <div class="ic">${I.pin()}</div>
    <b>${MSCOPE.all ? `No nail techs in ${esc(MSCOPE.state || "that state")} yet`
                    : "No nail techs near you yet"}</b>
    Oma shows techs who have listed themselves and passed their ID check.
    ${out ? `<div style="margin-top:10px"><button class="lnk" style="margin-left:0"
      data-a="map-elsewhere" data-v="${esc(b.state)}">See other states</button></div>` : ""}
    ${MSCOPE.all ? `<div style="margin-top:10px"><button class="lnk" style="margin-left:0"
      data-a="map-scope" data-v="near">Back to near me</button></div>` : ""}
  </div>`;
}
...
function homeAsList() {
  ...
  fillHost(`
    <div class="maplist">
      <!-- The big pink "Hand scan" card was here and is gone — Kamsy,
           16 Sep 2026. Scanning is not orphaned by that: the pink button in
           the middle of the bottom bar does the same thing, from every screen
           rather than only this one, and it is the largest target in the app.
           The card was a second front door standing next to the first,
           spending the whole top of home on a thing you can already reach
           with your thumb without looking.

           The MAP version of home never had one; this is the fallback screen,
           and the two now agree about what the top of home is for. -->
      ${last ? `<div class="pad mt16">${scanCard(last)}</div>` : ""}
