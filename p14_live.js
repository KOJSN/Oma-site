  ...
  load(async () => {
    ...
    fillHost(`
      <div class="pad stack gap12">
        ...
        <!-- Both sides, through the same door, and it stays open after the
             appointment is over — most people do not report while it is
             ...
             on, so it disappears exactly there. Before release — including
             a no-show, which is reported here as "Did not turn up" — the
             door stays open, because a no-show is exactly the kind of
             problem this button exists for. Also gone before payment, for
             the same "nothing has happened yet" reason as the chat button
             above. -->
        ${["awaiting_payment", "released"].includes(b.status) ? "" : `
